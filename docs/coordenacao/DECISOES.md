...
### D15 — Codex descontinuado; Claude assume as duas funções (usuário, 2026-07-17)
O usuário decidiu parar de usar o Codex neste projeto ("está muito ruim ultimamente"). A partir de agora, Claude acumula os dois papéis definidos em D12: líder técnico (código, PDFs finais, site, testes, publicação, integração, decisões) **e** preparação (pesquisa de fontes, download de materiais permitidos, mapeamento de links diretos do Poliedro, transcrição de respostas finais mínimas com evidência, identificação de lacunas). O fluxo `PEDIDOS_PARA_CODEX.md` → `ENTREGAS_DO_CODEX.md` deixa de ser necessário como troca entre agentes, mas os dois arquivos continuam servindo de registro histórico e de checklist de tarefas pendentes — passam a ser preenchidos por Claude diretamente, sem a etapa de "pedido/entrega" formal. As mesmas regras de qualidade e conferência (segunda verificação por amostragem, hash, fidelidade visual, evidência por resposta) continuam valendo integralmente — não há redução de rigor, só de intermediação.

### D14 — Integração de pacotes por questão, não por item (Claude, 2026-07-17, ao retomar)
O integrador (`ferramentas/integrar_pacote.py`) publicava um item inteiro somente se TODAS as suas questões estivessem `status_questao == "pronto"`. Na prática isso reteve 5 de 6 provas discursivas 2017–2018 inteiras (39 respostas prontas) porque cada uma tinha 1–3 questões pendentes. Isso não é exigido pelo plano: a seção 16 já prevê, por questão, a frase padrão "Confira a resolução completa..." para quando não há resultado separável — ou seja, o desenho original já assume provas parcialmente prontas. Reprojetei o integrador para publicar por questão: cada resposta usa seu próprio `status_questao`; "pronto" publica o texto real, qualquer outro valor publica a frase padrão (nunca o texto candidato não confirmado). O CSV de auditoria (`RESPOSTAS_COMPLEMENTARES.csv`) continua guardando o texto original do Codex para toda questão, pronta ou não, para rastreabilidade — só o PDF público é que filtra. Este é o comportamento definitivo para todos os pacotes futuros do Codex com granularidade por questão.

### D16 — Novas diretrizes do usuário para destravar transcrições (2026-07-17)
O usuário determinou: (1) quando a notação de uma resposta for complexa/ambígua demais para transcrição textual segura, **embutir um recorte de imagem apenas do resultado final** (nunca do desenvolvimento) na seção de respostas do PDF — o crédito ao Poliedro já consta na página de proteção; o recorte não é commitado no repositório, só embutido no PDF final; (2) questões que o Poliedro recomenda anular entram com a resposta do Poliedro seguida de "(o Poliedro recomendou a anulação desta questão)"; (3) para materiais sem fonte oficial, usar a melhor fonte disponível em vez de travar, registrando a procedência; (4) decisões tomadas pelo agente sozinho, usuário corrige depois se discordar.
